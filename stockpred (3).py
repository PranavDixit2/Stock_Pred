...
def preprocess_data(data):
    # FIX: Correctly calculate and align Log Returns
    # 1. Calculate the Log_Return for the entire dataset
    log_returns = np.log(data['Close'] / data['Close'].shift(1))

    # 2. The target for features(t) is Log_Return(t+1). Shift the Series back.
    ...
    # 4. Drop any remaining NaNs in the target (the last row) and align the feature data
...
def plot_predictions(data_for_lstm, y_test_actual, y_pred, lower_bound, upper_bound):
    # Test dates are the dates corresponding to Price_t+1 (the target)
...
def predict_next_day(model, data_processed, features, feature_scaler, target_scaler, window_size):
    # Data for the lookback window (up to and including the last day, t)
    ...
    # Predict the next-day log return (t+1)
    ...
    # Get today's closing price (Price_t)
...
# IMPROVEMENT: Increased default lookback window from 10 to 20
...
if st.button("Run Prediction (Improved Model)"):
    ...
    # Ensure files are saved uniquely for the different model target
    ...
    with st.spinner("Preprocessing data..."):
        ...
        # data_for_lstm now only needs to contain the features (including 'Close')
        # aligned with the target for the prepare_data step.
    ...
    # data_for_inversion contains the 'Close' values needed to convert
    # the scaled log returns back to actual prices for the test set.
    # We need the last WINDOW_SIZE + len(y_test) rows of the feature-engineered data for correct inversion
    data_for_inversion = data_for_lstm.iloc[-(WINDOW_SIZE + len(y_test)):].copy()
    ...
    if os.path.exists(model_path) and os.path.exists(scaler_path):
        ...
        model = Sequential()
        # IMPROVEMENT: L2 Regularization reduced from 0.001 to 0.0005
    ...
    # We pass the full, aligned data_for_lstm here for the next-day prediction
